...
void create_debris(ChIrrAppInterface& application, double dt, double particles_second, ISceneNode* parent)
{
	...
	video::ITexture* bluwhiteMap = application.GetVideoDriver()->getTexture("../objects/bluwhite.png");
	video::ITexture* pinkwhiteMap = application.GetVideoDriver()->getTexture("../objects/pinkwhite.png");
	...
	for (int i = 0; i < particles_dt; i++)
	{
		...
		if (rand_fract < sph_fraction)
		{
			...
			mrigidBody = (ChBodySceneNode*)addChBodySceneNode_easySphere(
												...
												sphmass,
												//ChVector<>(-0.5*xnozzlesize+ChRandom()*xnozzlesize+xnoozle, ynozzle+i*0.005, 0),
												ChVector<>(-0.5*xnozzlesize+ChRandom()*xnozzlesize+xnozzle, conv_thick/2+sphrad, -0.5*znozzlesize+ChRandom()*znozzlesize),
												sphrad, 
												15,8, parent);

			mrigidBody->GetBody()->SetInertiaXX(ChVector<>(sphinertia,sphinertia,sphinertia));
			mrigidBody->GetBody()->SetFriction(0.2f); 
			mrigidBody->GetBody()->SetImpactC(0.8f); 
			//mrigidBody->GetBody()->GetCollisionModel()->SetFamily(5);
			//mrigidBody->GetBody()->GetCollisionModel()->SetFamilyMaskNoCollisionWithFamily(3); 
			mrigidBody->addShadowVolumeSceneNode();
			mrigidBody->setMaterialTexture(0,	bluwhiteMap);

			// Here is how to create and add a custom asset to the object! ***ALEX
			ChSharedPtr<ElectricParticleProperty> electric_asset(new ElectricParticleProperty); 
			mrigidBody->GetBody()->AddAsset(electric_asset);
		...
		//	ChCollisionModel::SetDefaultSuggestedMargin  (0.002);
			...
			mrigidBody = (ChBodySceneNode*)addChBodySceneNode_easyBox(
												application.GetSystem(), application.GetSceneManager(),
												sphmass,
												ChVector<>(-0.5*xnozzlesize+ChRandom()*xnozzlesize+xnozzle, ynozzle+i*0.005, -0.5*znozzlesize+ChRandom()*znozzlesize),
		...
		//	ChCollisionModel::SetDefaultSuggestedMargin  (0.002);
			...
			mrigidBody = (ChBodySceneNode*)addChBodySceneNode_easyCylinder(
												application.GetSystem(), application.GetSceneManager(),
												cylmass,
												ChVector<>(-0.5*xnozzlesize+ChRandom()*xnozzlesize+xnozzle, conv_thick/2+2*sphrad, -0.5*znozzlesize+ChRandom()*znozzlesize),
												//ChQuaternion<>(pow(2,0.5)/2,0,0,pow(2,0.5)/2), //cyl normal
												ChQuaternion<>(pow(2,0.5)/2,pow(2,0.5)/2,0,0), //cyl parallel
												//ChQuaternion<>(1,0,0,0), //disk normal
												ChVector<>(sphrad*2,cylhei,sphrad*2),
												parent);

			//mrigidBody->GetBody()->SetInertiaXX(ChVector<>(sphinertia,sphinertia,sphinertia));
			mrigidBody->GetBody()->SetInertiaXX(ChVector<>(cylinertia,cylinertia2,cylinertia));
		...
		{
			mrigidBody->GetBody()->SetLimitSpeed(true);
			mrigidBody->GetBody()->SetMaxSpeed(100);
			mrigidBody->GetBody()->SetMaxWvel(250);
		}
...
void purge_debris(ChIrrAppInterface& application, ISceneNode* parent, int nmaxparticles = 500)
{
	...
	for (; it != parent->getChildren().end(); ++it)
	{
		...
		// Delete particles that are too many (beyond the nmaxparticles limit)
		if (parent->getChildren().size() > nmaxparticles)
			deletion = true;

		if (ChBodySceneNode* mybody = dynamic_cast<ChBodySceneNode*>(todelete))
		{
			// Delete particles that are too far ***ALEX
			if ( fabs(mybody->GetBody()->GetPos().x) > 20 ||
				 fabs(mybody->GetBody()->GetPos().y) > 20 ||
				 fabs(mybody->GetBody()->GetPos().z) > 20  ) 
			{
				deletion = true;
			}
			/*
			// Delete particles that are spinning too fast! ***ALEX
			if ( mybody->GetBody()->GetWvel_loc().Length() > 350 ) 
			{
				GetLog() << "D! ";
				deletion = true;
			}
		...
		if (it == parent->getChildren().end()) break; // avoid 0 null pointer if this was the last particle
	}
}
...
int main(int argc, char* argv[])
{
	...
	application.AddTypicalCamera(core::vector3df(1.5,0.4,-1), core::vector3df(0,0,0));
	...
	// HOW TO IMPORT A SOLIDWORK MODEL 
...
*/
	...
	while(application.GetDevice()->run()) 
	{
		...
		{ 
			...
			purge_debris (application, parent, 10000);
